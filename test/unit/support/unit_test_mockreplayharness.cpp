...
namespace AqualinkAutomate::Test
{
	...
	MockReplayHarness::MockReplayHarness(bool single_read_per_poll)
		: m_HubLocator()
			...
			std::unique_ptr<Test::TestSerialPortImpl>(m_SerialImplPtr), m_HubLocator))
	{
		...
		// single_read_per_poll is false by default (drain per poll) so existing
		// tests run at full speed; replay-pacing tests pass true.
		m_ProtocolTask = std::make_shared<Protocol::ProtocolTask>(m_SerialPort, m_StatisticsHub, single_read_per_poll);
	}
