...
namespace AqualinkAutomate::HTTP::Router
{
	...
	node const* impl::find_optional_resource(const node* root, std::vector<node> const& ns, std::string_view*& matches, std::string_view*& ids)
	{
		BOOST_ASSERT(root);
		if (root->resource)
		{
	...
	void impl::insert_impl(std::string_view path, router_base::any_resource const* v)
	{
	...
	node const* impl::try_match(boost::urls::segments_encoded_view::const_iterator it, boost::urls::segments_encoded_view::const_iterator end, node const* cur, int level, std::string_view*& matches, std::string_view*& ids) const
	{
		while (it != end)
		{
