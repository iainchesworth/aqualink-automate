...
namespace AqualinkAutomate::Generators
{
	...
	Types::JandyExpectedMessageType GenerateMessageFromRawData(boost::circular_buffer<uint8_t>& serial_data)
	{
		...
		{
			boost::circular_buffer<uint8_t>::iterator packet_one_start_it, packet_one_end_it, packet_two_start_it;
			...
			else
			{
				...
				else
				{
					...
					//FIXME auto packet_length = std::distance(packet_one_start_it, packet_one_end_it) + 2; // Account for the DLE,ETX bytes
					//FIXME auto message_span = std::as_bytes(std::span(packet_one_start_it, packet_length));
					//FIXME LogTrace(Channel::Messages, std::format("Aqualink packet (length: {} bytes) found in serial data.", message_span.size()));

					// Step 3 -> Validate that the packet is correctly formed and has not been corrupted
					if (!PacketValidation_ChecksumIsValid(packet_one_start_it, packet_one_end_it + 2))
					{
					...
					else
					{
						...
						//FIXME auto message_span = std::as_bytes(std::span(packet_one_start_it, packet_one_end_it + 2));
						auto message_range = std::ranges::subrange(packet_one_start_it, packet_one_end_it + 2);
