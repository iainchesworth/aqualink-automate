...
namespace AqualinkAutomate::Developer
{
	...
	{
		LogTrace(Channel::Developer, std::format("Construction of type: {} -> total instances: {}", object_type_name, total_instances));
	}

	void LoggingReporter::ReportDestruction(std::string_view object_type_name, std::size_t total_instances) const noexcept
	{
		LogTrace(Channel::Developer, std::format("Destruction of type: {} -> remaining instances: {}", object_type_name, total_instances));
	}
