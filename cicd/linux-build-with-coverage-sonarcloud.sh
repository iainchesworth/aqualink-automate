...
CMAKE_PRESET=${1:-"config-linux-gcc-coverage"}
...
build-wrapper-linux-x86-64 --out-dir build_wrapper_output_directory cmake --build --preset=build-linux-gcc-coverage
...
sonar-scanner ${SONAR_DEBUG_CMD_ARG} ${SONAR_URL_CMD_ARG} ${SONAR_TOKEN_CMD_ARG} ${SONAR_OTHER_ARGS} ${SONAR_GCOVR_COVERAGE}
