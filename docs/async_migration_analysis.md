...
### 6.2 MQTT is hand-rolled — no library to lean on
...
1. **Single-thread forever, or a future (B)?** This plan commits to (A) and treats (B) as a separate profiling-gated effort. If multi-thread is ever a hard requirement, the strand-confine checklist (static signal, `Publish`, every doc-commented container) must be executed first — decide whether to pre-invest in those asserts now.
