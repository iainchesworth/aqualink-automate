...
The Dockerfile is multi-stage (`base`, `dev`, `ci`, `matter-builder`, `runtime`); the default build target is `runtime`. The runtime image is built on `ubuntu:26.04` with `tini` as PID 1, Node.js 22 for the Matter bridge sidecar, and `curl` for the container health check. It runs as an unprivileged `aqualink` user (uid/gid 10000), exposes port 80, declares a `HEALTHCHECK` against `/api/health`, and its default command is `--address 0.0.0.0 --disable-https`.
