...
| `.github/workflows/docs.yml` | Workflow | Build the docs site (`zensical build --strict`) and publish it to the root of `gh-pages` on docs-affecting pushes to `main`. |
...
The documentation site is built with **Zensical** — the Rust-core successor to MkDocs + Material (which entered maintenance mode ahead of its 2026-11-05 EOL); Zensical reads the same `mkdocs.yml`, including the `nav` that selects the published subset. It is built from the in-repo Markdown and published to the **root** of the `gh-pages` branch, so the docs home is the GitHub Pages landing page.

- **Triggers:** push to `main` touching `docs/**`, `overrides/**`, `mkdocs.yml`, `scripts/zensical-prune-excluded.py`, `README.md`, or the workflow itself; plus `workflow_dispatch`.
- **Build:** `pip install zensical`, then `zensical build --strict` — broken internal links/anchors and other warnings **fail the build** instead of shipping a broken page. `theme.variant: classic` in `mkdocs.yml` keeps the Material appearance the brand skin (`overrides/`, `docs/stylesheets/aqualink.css`) is built for; Zensical's own default is a different, newer theme.
- **`exclude_docs` is not honoured by Zensical** (as of 0.0.60) — it builds every page under `docs_dir` regardless, so `design/`, `refactoring/`, `alwin32/`, and the other entries listed in `mkdocs.yml`'s `exclude_docs` block get built as real, unlinked (not in `nav`) pages. The **Prune docs excluded from the published site** step runs `scripts/zensical-prune-excluded.py` immediately after the build to delete them from `site/` and strip any stray `sitemap.xml` entries, reading the same `exclude_docs` list so it can't drift out of sync. Drop that step once Zensical gains native `exclude_docs` support.
- **Linking to files outside `docs/`, or into excluded content.** `--strict` resolves every relative link against the *documentation* tree, not the repository, and (because of the point above) against the *unpruned* build — a link to `design/ha-companion-package.md` resolves fine at build time and only 404s after the prune step runs. A link like `../docker-compose.yml` similarly 404s on the published site even though it resolves on GitHub. Use an absolute `https://github.com/iainchesworthlabs/aqualink-automate/blob/main/…` URL for both repo files outside `docs/` and for anything held back by `exclude_docs`; `zensical build --strict` will not catch either case, so read the prune step's summary line too.
- **No social-preview cards.** The old `social` plugin (Open-Graph/Twitter card generation) isn't supported by Zensical yet; it was dropped along with the MkDocs Material dependency rather than kept as dead config. Revisit if Zensical adds native support.
